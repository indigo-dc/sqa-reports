...
\vspace{-3em}
\begin{center}
    \graybox{\strut SQA Progress Status}\greenbox{\strut COMPLETE}\\[0.5em]\greenbox{\strut \begin{huge}83\% done\end{huge}} 
\end{center}
...
\begin{tabular}{ll}
    ...
    \hyperref[sec:unit_test]{Code coverage} & \graybox{46\%}\\ 
...
\begin{ganttchart}[
    ...
    \ganttgroup[progress=83]{\textbf{\#4016} jSAGA RM API SQA FOR 1ST RELEASE}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
...
Repository exists under \texttt{indigo-dc} GitHub organization:
    \begin{itemize}
        \item \url{https://github.com/indigo-dc/jsaga.git}\
        \end{itemize}
...
\href{https://jenkins.indigo-datacloud.eu:8080//job/jsaga-rm-codestyle/32}{jsaga-rm-codestyle}.
...
\includegraphics[scale=0.5]{/srv/sqa-reports/build/figs/graph_jsaga-rm-unittest.png}
\\
%\vspace{0.5em}
...
\begin{ganttchart}[
    ...
    %\ganttgroup[progress=45]{Group 1}{1}{12} \\
            \ganttbar[progress=84.78261]{Packages (39.0/46.0)}{1}{3} \\
            \ganttbar[progress=76.37131]{Files (181.0/237.0)}{1}{3} \\
            \ganttbar[progress=43.844494]{Classes (203.0/463.0)}{1}{3} \\
            \ganttbar[progress=33.26717]{Methods (838.0/2519.0)}{1}{3} \\
            \ganttbar[progress=33.826168]{Lines (3203.0/9469.0)}{1}{3} \\
            \ganttbar[progress=31.094452]{Conditionals (1037.0/3335.0)}{1}{3} \\
     [grid]
\end{ganttchart}
\end{center}

\subsection{Build status}
    Last build status on Jenkins CI \href{https://jenkins.indigo-datacloud.eu:8080//job/jsaga-rm-unittest/32}{jsaga-rm-unittest}.
...
\subsection{Build status}
Last build status on Jenkins CI
    \href{https://jenkins.indigo-datacloud.eu:8080//job/jsaga-rm-functional/19}{jsaga-rm-functional}.
    


\subsection{Observations}
\begin{itemize}
        \item Current unit tests contain integration checks. Will be separated and completed.
        \item The functionnality being tested will be the middleware-independant API to manage jobs (submission and monitoring), data management (logicial and physical files) and resource management (computer/storage dynamic allocation).
    \end{itemize}
...
\subsection{Observations}
\begin{itemize}
        \item Points to improve in the documentation: \begin{itemize} \item README: describe the role of the component within INDIGO infrastructure, other components interacting with it, etc. \item Deployment guide: describe the guidelines to deploy the component, mainly how to build it from INDIGO repository (tarball) \end{itemize}
    \end{itemize}
