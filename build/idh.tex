...
\title{Software Quality Assurance (SQA) Report \\[0.7em] \large{22-26 Aug 2016}\\[1.5em] 
       \huge{\texttt{idh (INDIGO-DataCLoud Identity-Harmonization Service)}}\\
}
...
\begin{tabular}{ll}
    ...
    \hyperref[sec:unit_test]{Code coverage} & \graybox{54\%}\\ 
...
\begin{ganttchart}[
    ...
    \ganttgroup[progress=83]{\textbf{\#2876} idh SQA FOR 1ST RELEASE}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
        ]{\textbf{\#3950} Repository synchronization}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
        ]{\textbf{\#3953} Code style specification}{1}{12} \\
            \ganttbar[
            progress=54,
            name=WBS1A
        ]{\textbf{\#2879} Unit testing coverage}{1}{12} \\
            \ganttbar[
            progress=90,
            name=WBS1A
        ]{\textbf{\#2894} Functional and integration testing coverage}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
        ]{\textbf{\#2882} Configuration management}{1}{12} \\
            \ganttbar[
            progress=50,
            name=WBS1A
        ]{\textbf{\#2885} GitBook documentation}{1}{12} \\
     [grid]
\end{ganttchart}
...
Repository exists under \texttt{indigo-dc} GitHub organization:
    \begin{itemize}
        \item \url{https://github.com/indigo-dc/identity-harmonization.git}\
        \end{itemize}
...
\href{https://jenkins.indigo-datacloud.eu:8080//job/idh-codestyle/18}{idh-codestyle}.
...
\subsection{Build status}
Last build status on Jenkins CI
    \href{https://jenkins.indigo-datacloud.eu:8080//job/idh-unittest/18}{idh-unittest}.
    


\subsection{Observations}
\begin{itemize}
        \item Missing list of functionalities tested. Will be provided in time.
    \end{itemize}


\input{templates/title_gitbook.tex}
\label{sec:gitbook}
Documentation available under \texttt{indigo-dc} GitBook organization: \vspace{0.1em} \begin{center}\url{https://indigo-dc.gitbooks.io/identity-harmonization/content/}\end{center} 
...
\subsection{Observations}
\begin{itemize}
        \item Points to improve in the documentation: \begin{itemize} \item README, describe: \begin{itemize} \item The purpose of the component, what it does, why is it valuable for, etc. \item How does it interact with other services, does it have dependencies on other services? does it use other services or components? where should it be deployed ? and for what ? \end{itemize} \item Deployment guide: \begin{itemize} \item Installation using packages: make use of INDIGO repository and apt-get/yum commands instead \item How is this service configured and used ? Explain configuration parameters at \texttt{/var/lib/identity-harmonization/config/application.yml} \item If applicable, indicate the TCP ports that need to be exposed and to which services \end{itemize} \item Developer guide: not complete \item API walkthrough not complete \end{itemize}
    \end{itemize}
...
\begin{tabular}{ll}
    ...
    Manifest link & \url{https://github.com/indigo-dc/ansible-role-idh-master} \\
\end{tabular}
\end{center}
