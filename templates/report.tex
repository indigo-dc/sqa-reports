...
\begin{tabular}{ll}
    ...
    %\hyperref[sec:func_int_test]{Functional/integration testing} & \BLOCK{if product.func_int_test.wp3_in_progress}\graybox{IN PROGRESS - WP3}\BLOCK{elif product.func_int_test.url}\greenbox{COMPLETE}\BLOCK{elif product.func_int_test.jobs}\greenbox{COMPLETE}\BLOCK{else}\redbox{NOT COMPLETE}\BLOCK{endif} \\
    ...
    \hyperref[sec:gitbook]{GitBook documentation} & \BLOCK{if product.gitbook.url}\greenbox{COMPLETE}\BLOCK{else}\redbox{NOT COMPLETE}\BLOCK{endif} \\
...
\BLOCK{elif product.config_management.tool}
\subsection{Build status}
\begin{center}
\begin{tabular}{ll}
