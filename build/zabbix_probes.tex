...
\vspace{-3em}
\begin{center}
    \graybox{\strut SQA Progress Status}\redbox{\strut NOT COMPLETE}\\[0.5em]\redbox{\strut \begin{huge}73\% done\end{huge}} 
\end{center}
...
\begin{ganttchart}[
    ...
    \ganttgroup[progress=73]{\textbf{\#3221} Zabbix Probes SQA FOR 1ST RELEASE}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
        ]{\textbf{\#3224} Repository synchronization}{1}{12} \\
            \ganttbar[
            progress=100,
            name=WBS1A
        ]{\textbf{\#3845} Code style specification}{1}{12} \\
            \ganttbar[
            progress=50,
            name=WBS1A
        ]{\textbf{\#3233} Unit testing coverage}{1}{12} \\
            \ganttbar[
            progress=90,
            name=WBS1A
        ]{\textbf{\#3236} Functional and integration testing coverage}{1}{12} \\
            \ganttbar[
            progress=50,
            name=WBS1A
        ]{\textbf{\#3239} Configuration management}{1}{12} \\
            \ganttbar[
            progress=50,
            name=WBS1A
        ]{\textbf{\#3242} GitBook documentation}{1}{12} \\
     [grid]
\end{ganttchart}
...
Repository exists under \texttt{indigo-dc} GitHub organization:
    \begin{itemize}
        \item \url{https://github.com/indigo-dc/Zabbix-probes.git}\
        \end{itemize}
...
\href{https://jenkins.indigo-datacloud.eu:8080//job/zabbix-probes-codestyle/13}{zabbix-probes-codestyle}.

\subsection{Observations}
\begin{itemize}
        \item Progress set to 70\% since the product is adhered to a standard, but not currently compliant with it.
    \end{itemize}
...
\subsection{Build status}
    Last build status on Jenkins CI \href{https://jenkins.indigo-datacloud.eu:8080//job/zabbix-probes-unittest/5}{zabbix-probes-unittest}.
    
\subsection{Observations}
\begin{itemize}
        \item Command/s to execute the Jenkins job has been provided.
    \end{itemize}
...
\subsection{Build status}
Last build status on Jenkins CI
    \href{https://jenkins.indigo-datacloud.eu:8080//job/zabbix-probes-unittest/5}{zabbix-probes-unittest}.
    

\subsection{Reports}
Functional report/s available:
\begin{itemize}
        \item \href{https://owncloud.indigo-datacloud.eu/index.php/apps/files/?dir=%2FINDIGO-DC%20testing%20reports%2Fintegration%2Findigo_1%2FZabbix%20Probes%20-%20Monitoring}{owncloud report}
    \end{itemize}
...
\label{sec:gitbook}
Documentation available under \texttt{indigo-dc} GitBook organization: \vspace{0.1em} \begin{center}\url{https://www.gitbook.com/book/indigo-dc/monitoring/details}\end{center} 
\subsection{Types of documentation currently provided}
\begin{center}
\graybox{\strut Readme}
\graybox{\strut Deployment and Administration documentation}
\graybox{\strut Developer documentation}
\graybox{\strut User documentation}
\end{center}
\subsection{Observations}
\begin{itemize}
        \item Points to improve in the documentation: \begin{itemize} \item Section \texttt{Zabbix scripts} is empty. \item Installation subsection in \texttt{Zabbix probes} is empty. \item Describe the installation steps using packages from INDIGO repository. \item Describe the deployment of Docker container using dockerhub's indigodatacloud/monitoring repository. \item Elaborate more on the role within INDIGO architecture, what services are monitored, etc. \item The configuration in \texttt{Zabbix probes} is hard to follow, provide table with parameters and examples. \begin{itemize} \item Provide examples of valid configurations. \end{itemize} \end{itemize}
    \end{itemize}
...
\subsection{Observations}
\begin{itemize}
        \item Ansible playbooks are being generated.
    \end{itemize}
